...
<script>
import {reqOrderPay,reqOrderStatus} from "@/api"
...
export default {
  ...
  methods:{
    async getData(){
      let result = await reqOrderPay(this.$route.query.orderId)
      if(result.code == 200){
    ...
    async pay(){
      ...
      this.timer = setInterval(async ()=>{
        let result = await reqOrderStatus(this.payInfo.orderId);
        if (result.code == 200){
          console.log("支付状态",result.data)
